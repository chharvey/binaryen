...
export function table(mod: Module) {
	return {
		...
		get: (name: string, index: number, typ: Type): ExpressionRef => (
			preserveStack(() => BinaryenObj["_BinaryenTableGet"](mod[PTR], strToStack(name), index, typ) as ExpressionRef)
		),

		/** Store an element in a table. */
		set: (name: string, index: number, value: ExpressionRef): ExpressionRef => (
			preserveStack(() => BinaryenObj["_BinaryenTableSet"](mod[PTR], strToStack(name), index, value) as ExpressionRef)
		),

		/** Returns the current size of a table. */
		size: (name: string): ExpressionRef => (
			preserveStack(() => BinaryenObj["_BinaryenTableSize"](mod[PTR], strToStack(name)) as ExpressionRef)
		),
		...
		grow: (name: string, value: ExpressionRef, delta: ExpressionRef): ExpressionRef => (
			preserveStack(() => BinaryenObj["_BinaryenTableGrow"](mod[PTR], strToStack(name), value, delta) as ExpressionRef)
		),
